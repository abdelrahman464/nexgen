...
// * Get section from params and check if user is admin or course instructor
exports.getSectionAndCheckInstructor = async (req, res, next) => {
  try {
    ...
    // Find the section and populate the course
    ...
    if (!section.course) {
      return res.status(404).json({
        status: "failed",
        error: "Course not found for this section",
      });
    }
    ...
    // Check if current user is the instructor of the course that contains this section
    if (section.course.instructor.toString() === req.user._id.toString()) {
      return next();
    }

    // User is neither admin nor instructor of the course
    return res.status(403).json({
      status: "failed",
      error: "You are not authorized ",
    });
