...
// create route to get all users that have alive package subscription
//@route GET /api/v1/users/users-with-alive-package-subscription
//@access private admin
exports.getUsersWithAlivePackageSubscription = async (req, res, next) => {
  try {
    const subscriptions = await UserSubscription.find({
      ...
      .select(
        "user package startDate endDate instructor subscriptionDurationDays",
      );
    // Localize list response using the array-safe schema helper.
    const localizedSubscriptions =
      UserSubscription.schema.methods.toJSONLocalizedOnly(
        subscriptions,
        req.locale,
      );
    res.status(200).json({
      status: "success",
      results: localizedSubscriptions.length,
...
//function to get all instuctor that have active courses
//@route GET /api/v1/users/instructorsWithActiveCourses
//@access private admin
exports.getAllInstructorsWithActiveCourses = async (req, res, next) => {
  try {
    const instructorIds = await Course.distinct("instructor", {
      status: "active",
    });
    // return usefull instructor data
    const instructors = await User.find({
      _id: { $in: instructorIds },
      isInstructor: true,
    }).select("name email profileImg bio");
