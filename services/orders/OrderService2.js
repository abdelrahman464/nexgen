...
async function addUserToGroupChat(userId, courseId) {
  // First check if user already exists in the chat
  ...
  if (existingChat) {
    // User already exists in the chat, no need to add them again
    return existingChat;
  }

  // User doesn't exist in chat, add them
...
const createCoursePackageOrder = async (id, userId, isPaid) => {
  ...
  if (isPaid) {
    ...
    await handleOrderCommissions(coursePackage, data);
  }
  return true;
};
//2
const createPackageOrder = asyncHandler(async (id, userId, isPaid) => {
  ...
  if (isPaid) {
    ...
    // await calculateProfits(data);
    await handleOrderCommissions(package, data);
  }
  return true;
});
//3
