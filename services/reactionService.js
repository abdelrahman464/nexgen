...
exports.addReact = asyncHandler(async (req, res, next) => {
  ...
  if (existingReaction) {
    ...
    existingReaction.type = type;
    await existingReaction.save();
    res.status(200).json({ message: "updated", data: existingReaction });
  } else {
    // Create and save a new reaction with population
    let react = await Reaction.create({ post: postId, user: userId, type });
    react = await Reaction.findById(react._id);
    return res.status(201).json({ message: "added", data: react });
  }
});
