...
exports.signup = asyncHandler(async (req, res, next) => {
  ...
  if (req.body.invitor) {
    ...
    invitor = await User.findById(req.body.invitor);
    if (!invitor) {
      return next(new ApiError("invitor is not valid", 400));
    }
    if (invitor.isTreeHead === true) {
      treeHead = invitor._id;
    } else if (invitor.treeHead) {
      // eslint-disable-next-line prefer-destructuring
      treeHead = invitor.treeHead;
    }
    invitor = invitor._id;
  }
  //create user
...
<body>
  ...
  try {
    ...
    return next(
      new ApiError(
        "There was a problem sending the email with your reset code",
        500
      )
    );
  }
